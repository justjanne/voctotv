...
const val FILTER_KEY_OTHER = "Other"

fun calculateTracks(conference: ConferenceModel, lecture: LectureModel): List<String> {
    val tags =
        lecture.tags
            .filter { !it.all(Char::isDigit) }
            .filter { !it.startsWith("${conference.acronym}-") }
            .filter { it != conference.acronym }
    return tags.ifEmpty { listOf(FILTER_KEY_OTHER) }
}
